...
export function DoctorSchedule({
    ...
    const handleSlotClick = (day: string, slot: TimeSlot) => {
        if (!slot.isBooked) {
            ...
            router.push(
                `/${locale}/booking/${doctorId}/${encodeURIComponent(dateStr)}`
            )
        }
    }
    ...
    const findNextAvailableDay = () => {
        ...
        while (!found && attempts > 0) {
            const daySchedule = schedule.find(
                (d) => d.day === format(currentDay, 'EEEE', { locale: hu })
            )

            if (
                daySchedule &&
                daySchedule.slots.some((slot) => !slot.isBooked)
            ) {
                found = true
    ...
    const selectedDaySchedule = schedule.find(
        (d) => d.day === format(selectedDate, 'EEEE')
    )
    const availableSlots =
        selectedDaySchedule?.slots.filter((slot) => !slot.isBooked) || []
